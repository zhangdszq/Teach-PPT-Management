    ...
    <div class="ml-64">
      ...
      <header class="bg-white shadow-sm border-b border-gray-200">
        <div class="flex items-center justify-between px-6 py-4">
          <div>
            ...
            <p class="text-sm text-gray-600 mt-1">{{ currentWorkspaceName }} - 管理所有PPT文件和项目</p>
          </div>
          ...
          <div class="flex items-center space-x-4">
            ...
            <router-link to="/create" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
              + 新建PPT
            </router-link>
      ...
      <div class="p-6">
        ...
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center space-x-4">
            ...
            <div v-if="selectedPPTs.length > 0" class="flex items-center space-x-2 bg-blue-50 px-3 py-2 rounded-lg">
              ...
              <button @click="batchPublish" class="text-sm text-blue-600 hover:text-blue-700 font-medium" :disabled="loading">批量发布</button>
              <button @click="batchArchive" class="text-sm text-blue-600 hover:text-blue-700 font-medium" :disabled="loading">批量归档</button>
              <button @click="batchDelete" class="text-sm text-red-600 hover:text-red-700 font-medium" :disabled="loading">批量删除</button>
              <button @click="clearSelection" class="text-sm text-gray-600 hover:text-gray-700">取消选择</button>
            </div>
            ...
            >
                ...
                <div class="absolute top-3 right-3">
                  <span
                    :class="[
                      ...
                      ppt.status === 'published' ? 'bg-green-500' : ppt.status === 'draft' ? 'bg-yellow-500' : 'bg-gray-500',
                    ]"
                  >
              ...
              <div class="p-4">
                ...
                <div class="flex items-center justify-between">
                  <button @click.stop="showPPTDetails(ppt)" class="text-blue-600 hover:text-blue-700 text-sm font-medium">查看详情</button>
                  <div class="flex items-center space-x-1">
                    <button @click.stop="editPPT(ppt)" class="text-blue-600 hover:text-blue-700 p-1" title="编辑">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                          stroke-linecap="round"
                          stroke-linejoin="round"
                    ...
                    <button @click.stop="previewPPT(ppt)" class="text-green-600 hover:text-green-700 p-1" title="预览">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="relative">
                      <button @click.stop="togglePPTMenu(ppt.id)" class="text-gray-600 hover:text-gray-700 p-1" title="更多操作">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                      ...
                      >
                        <button @click.stop="duplicatePPT(ppt)" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">
                          复制
                        </button>
                        <button @click.stop="downloadPPT(ppt)" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">下载</button>
                        <button @click.stop="sharePPT(ppt)" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">分享</button>
                        <hr class="my-1" />
                        <button @click.stop="deletePPT(ppt)" class="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50">删除</button>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="ppt in filteredPPTs" :key="ppt.id" class="hover:bg-gray-50 cursor-pointer" @click="selectPPT(ppt)">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div class="flex items-center space-x-2">
                        <button @click.stop="showPPTDetails(ppt)" class="text-blue-600 hover:text-blue-700" :disabled="loading">详情</button>
                        <button @click.stop="editPPT(ppt)" class="text-blue-600 hover:text-blue-700" :disabled="loading">编辑</button>
                        <button @click.stop="previewPPT(ppt)" class="text-green-600 hover:text-green-700" :disabled="loading">预览</button>
                        <button @click.stop="duplicatePPT(ppt)" class="text-orange-600 hover:text-orange-700" title="复制" :disabled="loading">
                          复制
                        </button>
                        <button @click.stop="downloadPPT(ppt)" class="text-indigo-600 hover:text-indigo-700" title="下载" :disabled="loading">
                          下载
                        </button>
                        <button @click.stop="sharePPT(ppt)" class="text-cyan-600 hover:text-cyan-700" title="分享" :disabled="loading">分享</button>
                        <div class="relative">
                          <button @click.stop="togglePPTMenu(ppt.id)" class="text-gray-600 hover:text-gray-700">⋯</button>
                          <div
                            v-if="activePPTMenu === ppt.id"
                            class="absolute right-0 top-8 w-32 bg-white rounded-md shadow-lg border border-gray-200 z-20"
                          >
        ...
        <div v-else-if="!loading" class="text-center py-12">
          ...
          <router-link to="/create" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors"> 创建PPT </router-link>
        </div>
      </div>
    </div>

    <!-- PPT详情弹窗 -->
    <div v-if="showDetailsModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click="showDetailsModal = false">
        ...
        <div class="p-6">
          ...
          <div v-if="selectedPPTDetails" class="space-y-6">
                    ...
                    <div class="mt-1">
                      <span
                        :class="[
                          ...
                          selectedPPTDetails.status === 'published'
                            ? 'bg-green-100 text-green-800'
                            : selectedPPTDetails.status === 'draft'
                            ? 'bg-yellow-100 text-yellow-800'
                            : 'bg-gray-100 text-gray-800',
                        ]"
                      >
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200">
              <button @click="showDetailsModal = false" class="px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
                关闭
              </button>
              <button
                @click="
                  previewPPT(selectedPPTDetails);
...
const filteredPPTs = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(
      (ppt) => ppt.name.toLowerCase().includes(query) || ppt.subject.toLowerCase().includes(query) || ppt.chapter.toLowerCase().includes(query),
    );
  }
